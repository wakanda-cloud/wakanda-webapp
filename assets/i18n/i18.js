...
$(document).ready(function() {
    ...
    let transformTranslateKey = function (i18nKey) {
        var i18n_translate = i18n(i18nKey.toLowerCase());
        let firstCharacter = i18nKey.substr(0, 1);
        if (firstCharacter === firstCharacter.toUpperCase()) {
            i18n_translate = i18n_translate.charAt(0).toUpperCase() + i18n_translate.slice(1);
        } else {
            i18n_translate = i18n_translate.charAt(0).toLowerCase() + i18n_translate.slice(1);
        }
        return i18n_translate;
    };

    var appendI18nKey = function (index, element) {
        var $element = $(element);
        if ($element.attr('i18n')) {
            $element.html(i18n($element.attr('i18n')));
        }
    };

    var replacePlaceholder_i18n = function (index, element) {
        var $element = $(element);
        if ($element.attr('placeholder_i18n')) {
            var placeholder_i18n = $element.attr('placeholder_i18n');
            var i18n_translate = transformTranslateKey(placeholder_i18n);

            $element.attr('placeholder', i18n_translate + "...");
        }
    };

    $.ajax("..\\assets\\i18n\\" + language + ".json").done(function(json) {
        i18n.translator.add(json);
        $('label').each(appendI18nKey);
        $('data').each(appendI18nKey);
        $('input').each(replacePlaceholder_i18n);
    });
});